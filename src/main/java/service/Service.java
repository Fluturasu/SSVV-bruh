...
public class Service {
    ...
    public int extendDeadline(String id, int noWeeks) {
        ...
        if (tema != null) {
            ...
            System.out.println("deadline=" + tema.getDeadline());
    ...
    public void createStudentFile(String idStudent) {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(student.getNume() + ".txt", false))) {
            notaXmlRepo.findAll().forEach(nota1 -> {
                if (nota1.getID().getObject1().equals(idStudent)) {
                    try {
                        bw.write("Tema: " + nota1.getID().getObject2() + "\n");
                        bw.write("Nota: " + nota1.getNota() + "\n");
                        bw.write("Predata in saptamana: " + nota1.getSaptamanaPredare() + "\n");
                        bw.write("Deadline: " + temaXmlRepo.findOne(nota1.getID().getObject2()).getDeadline() + "\n");
                        bw.write("Feedback: " + nota1.getFeedback() + "\n\n");
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
